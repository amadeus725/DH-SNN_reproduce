...
class DendriticDenseLayer(base.MemoryModule):
    ...
    def single_step_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 初始化状态
        if self.v is None:
            self.v = torch.zeros(batch_size, self.output_dim, device=x.device)
            self.spike = torch.zeros(batch_size, self.output_dim, device=x.device)
            self.d_current = torch.zeros(batch_size, self.output_dim, self.num_branches, device=x.device)
