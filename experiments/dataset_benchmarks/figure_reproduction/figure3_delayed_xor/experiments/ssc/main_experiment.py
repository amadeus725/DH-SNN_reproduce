#!/usr/bin/env python3
"""
SSC数据集上的DH-SNN实验 - Figure 4f的SSC部分
基于SpikingJelly框架，实现原论文的SSC语音命令识别任务
"""
...
print("🚀 SSC数据集DH-SNN实验 - Figure 4f")
print("="*60)

# SSC配置 - 按照原论文Table S5
SSC_CONFIG = {
    'learning_rate': 1e-2,
    'batch_size': 200,          # 原论文SSC使用200
    'epochs': 100,
    'hidden_size': 200,         # 原论文SSC使用200
    'output_size': 35,          # SSC有35个类别
    'v_threshold': 1.0,
    'dt': 1,
...
class SSC_ReadoutIntegrator(nn.Module):
    """SSC读出积分器"""
...
class SSC_VanillaSFNN(nn.Module):
    """SSC Vanilla SFNN"""
    ...
    def __init__(self, config, tau_m_range=(0.0, 4.0)):
        ...
        # 读出层
        self.readout = SSC_ReadoutIntegrator(
            config['hidden_size'],
            config['output_size'],
            tau_m_range,
            self.device
...
class SSC_DH_LIFNode(nn.Module):
    """SSC DH-LIF神经元"""

    def __init__(self, input_dim, output_dim, tau_m_range=(0.0, 4.0), tau_n_range=(2.0, 6.0),
                 num_branches=8, v_threshold=1.0, device='cpu'):  # SSC使用8个分支
        super().__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
...
def load_ssc_data(num_train=2000, num_test=500):
    """加载SSC数据"""
    print(f"📚 加载SSC数据: 训练{num_train}, 测试{num_test}")

    train_times, train_units, train_labels = read_gz_h5_file(
        "../spikingjelly_ssc/data/ssc_train.h5.gz", max_samples=num_train
    )
    test_times, test_units, test_labels = read_gz_h5_file(
        "../spikingjelly_ssc/data/ssc_test.h5.gz", max_samples=num_test
    )

    print("🔄 转换为张量...")

    # SSC数据的时间长度可能不同，需要检查
    train_data = torch.zeros(len(train_times), 1000, 700)  # 假设最大1000时间步
    ...
    print(f"📊 标签范围: 训练{train_labels.min()}-{train_labels.max()}, 测试{test_labels.min()}-{test_labels.max()}")
    return train_data, train_labels, test_data, test_labels

def train_ssc_model(model, train_data, train_labels, test_data, test_labels, config, model_name):
    """训练SSC模型"""
...
def main():
    ...
    try:
        ...
        # 加载SSC数据
        train_data, train_labels, test_data, test_labels = load_ssc_data(2000, 500)
        ...
        for timing_name, timing_config in TIMING_CONFIGS.items():
            ...
            # Vanilla SFNN
            vanilla_model = SSC_VanillaSFNN(SSC_CONFIG, tau_m_range=timing_config['tau_m'])
            vanilla_acc = train_ssc_model(
                vanilla_model, train_data, train_labels, test_data, test_labels,
                SSC_CONFIG, f"SSC Vanilla SFNN ({timing_name})"
            )

            # DH-SFNN
            dh_model = SSC_DH_SFNN(
                SSC_CONFIG,
                tau_m_range=timing_config['tau_m'],
                tau_n_range=timing_config['tau_n']
            )
            dh_acc = train_ssc_model(
                dh_model, train_data, train_labels, test_data, test_labels,
                SSC_CONFIG, f"SSC DH-SFNN ({timing_name})"
            )
        ...
        torch.save(results, "outputs/results/ssc_figure4f_results.pth")
...
if __name__ == '__main__':
    success = main()
    print(f"\n🏁 SSC实验完成，退出码: {0 if success else 1}")