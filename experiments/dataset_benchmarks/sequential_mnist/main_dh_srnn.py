#!/usr/bin/env python3
"""
Sequential MNIST DH-SRNN实验 - SpikingJelly版本
基于原论文配置实现
"""
...
def train_epoch(model, train_loader, criterion, optimizer, device, epoch):
    ...
    for batch_idx, (data, target) in enumerate(pbar):
        ...
        # 应用稀疏连接掩码 (如果是DH-SRNN)
        if hasattr(model, 'rnn1') and hasattr(model.rnn1, 'apply_mask'):
            model.rnn1.apply_mask()
            model.rnn2.apply_mask()
...
def main():
    parser = argparse.ArgumentParser(description='Sequential MNIST DH-SRNN实验')
    ...
    logger = setup_logger(f'{task_name}_{args.model_type}',
                         os.path.join(args.save_dir, f'{task_name}_{args.model_type}.log'))
    ...
    for epoch in range(args.epochs):
        ...
        if test_acc > best_acc:
            ...
            torch.save(model.state_dict(),
                      os.path.join(args.save_dir, f'{task_name}_{args.model_type}_best.pth'))
    ...
    torch.save(results, os.path.join(args.save_dir, f'{task_name}_{args.model_type}_results.pth'))
