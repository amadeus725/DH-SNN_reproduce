...
from config import *
from data_loader import load_ssc_data

def create_model(config):
    """创建模型"""
    
    if 'vanilla_sfnn' == 'vanilla_sfnn':
        model = VanillaSFNN(
    ...
    elif 'vanilla_sfnn' == 'vanilla_srnn':
        model = VanillaSRNN(
            input_size=config['input_size'],
            hidden_size=config['hidden_size'],
            output_size=config['output_size'],
        ...
        raise ValueError(f"Unknown model type: vanilla_sfnn")
...
def main():
    ...
    print(f"🏗️ 模型: VANILLA_SFNN")
    
    # 加载数据
    train_loader, test_loader = load_ssc_data(
    ...
    results = {
        'model_type': 'vanilla_sfnn',
        'best_accuracy': best_acc,
        'config': config
    }
    
    torch.save(results, f"{EXPERIMENT_CONFIG['output_dir']}/vanilla_sfnn_results.pth")
